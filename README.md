...
Lesson modules are Markdown files, one folder per language (`lessons/en`, `lessons/fa`), with
...
Hand-correction is not a finished stage but the work in progress: I am still reading through the
modules, and review continues. They are machine-drafted text under continuing human correction,
and no module should be taken as finally settled.
...
layer. That is a problem that a tool like Didaskalos is trying to solve, by helping a larger

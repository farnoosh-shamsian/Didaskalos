...
At its core, Didaskalos operates by integrating three key resources: a corpus of selected Ancient Greek texts, linguistic treebanks, and a modular grammar framework. The process begins when a learner chooses a target corpus. The system then analyzes this corpus using treebank data, identifying vocabulary frequency and morphological patterns. These linguistic features are ranked by how often they occur in the selected texts. The treebanks also carry syntactic annotation, but the syllabus makes little use of it so far — that is the clearest gap in the method as it stands.

Based on this analysis, Didaskalos constructs a frequency-driven learning path. Rather than following a predetermined order of topics, the system introduces vocabulary and grammar in descending order of how often each occurs in the chosen corpus, on the working assumption that this is the fastest route into those texts. That assumption is the project’s central bet, and it has not yet been tested against a conventional sequence.
...
One caveat worth stating: a sentence is unaltered, but it is an excerpt. It reaches you as a modern editor constituted it and as the treebank divided it, and lifted out of its paragraph it may open with a δέ or a γάρ answering something you cannot see.
...
The textbook is assembled modularly: grammar explanations are reusable units, one per linguistic phenomenon, and exercises are generated by extracting examples from the corpus. The explanatory modules began as drafts generated from standard reference grammars and have since been hand-corrected; review is ongoing. Because the pipeline’s data flow does not depend on the teaching language, the grammar modules can be translated and culturally adapted, which makes fully functional textbooks possible in languages — Persian among them — whose resources for learning Greek are very limited or nonexistent. What travels is that data flow rather than the explanatory framework: the modules assume the categories of the Western grammatical tradition, and a language whose own tradition carves things up differently may need more than a translation.

Didaskalos explores a corpus-centered model of language teaching: the syllabus is derived from texts rather than fixed in advance, and the texts are the learner’s own. The pedagogy is not derived from the corpus — it lives in the modules, the fixed opening, the ordering rules, and the difficulty scoring, all of which are editorial judgements. What the corpus decides is the order.

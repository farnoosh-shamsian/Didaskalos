---
topic: aorist, participle, middle (ω)
pos: verb
---

# The Aorist Middle Participle (ω-Verbs)

**Goal:**
By the end of this lesson you will be able to:

* Explain what an aorist participle is and how the middle voice works.
* Recognize and form the aorist middle participle (‑σάμενος / ‑όμενος).
* Use it in simple sentences and grasp its nuances of meaning.

---

## 1. What Is an Aorist Participle?

| Term | Meaning in Greek grammar | Rough approximation |
|------|--------------------------|------------------------------|
| **Aorist** | The aspect presenting the action as a simple, complete whole, without reference to duration. | "did / having done" |
| **Participle** | A verbal adjective: it both modifies a noun (like an adjective) and replaces a subordinate clause. | "having done…," "the one who…" |
| **Middle voice** | The subject is both doer and (often) beneficiary; the senses are reflexive, reciprocal, or "for oneself." | "washes himself," "fights for himself" |

The **aorist middle participle** therefore describes a *completed* action that the modified noun performed *for itself* (or with a middle nuance), and like an adjective it agrees with its noun in gender, number, and case.

---

## 2. Forms and Patterns

### 2.1 The Basic Shape

Aorist middle participle = **aorist stem (without the augment) + ‑σάμενος** (first aorist) or **+ ‑όμενος** (second aorist):

| Type | Masc. nom. | Fem. nom. | Neut. nom. |
|-------|----------------------|---------------------|-------------------|
| **First aorist** | **‑σάμενος** (πραξάμενος) | **‑σαμένη** (πραξαμένη) | **‑σάμενον** (πραξάμενον) |
| **Second aorist** | **‑όμενος** (γενόμενος) | **‑ομένη** (γενομένη) | **‑όμενον** (γενόμενον) |

### 2.2 Full Declension Table (Masculine Example)

The complete paradigm of the aorist middle participle of **πράττω** "to do, accomplish" (aorist stem **πραξ‑**):

| Case | Singular | Dual | Plural |
|------|----------|------|--------|
| **Nominative** | **πραξάμενος** | **πραξαμένω** | **πραξάμενοι** |
| **Genitive** | **πραξαμένου** | **πραξαμένοιν** | **πραξαμένων** |
| **Dative** | **πραξαμένῳ** | **πραξαμένοιν** | **πραξαμένοις** |
| **Accusative** | **πραξάμενον** | **πραξαμένω** | **πραξαμένους** |
| **Vocative** | **πραξάμενε** | **πραξαμένω** | **πραξάμενοι** |

> The dual is rare in Attic prose and shown only for completeness. The feminine declines like the first declension (πραξαμένη, πραξαμένης…), the neuter like δῶρον.

### 2.3 More Examples

| Verb | Aorist stem | Middle participle (masc. nom.) |
|------|-------------|--------------------------------|
| **λύω** "to loose" | λυσ‑ | **λυσάμενος** "having ransomed" |
| **δέχομαι** "to receive" | δεξ‑ | **δεξάμενος** "having received" |
| **γίγνομαι** "to become" | γεν‑ | **γενόμενος** "having become" |

### 2.4 Comparison with the Other Voices

| Voice | Aorist participle | Example |
|-------|---------------|----------------|
| **Active** | ‑σας, ‑σασα, ‑σαν / ‑ών | λύσας, λαβών |
| **Middle** | **‑σάμενος / ‑όμενος** | λυσάμενος, γενόμενος |
| **Passive** | ‑θείς, ‑θεῖσα, ‑θέν | λυθείς |

*The aorist distinguishes middle and passive with two separate formations; in the present, λυόμενος covers both meanings.*

---

## 3. Key Rules & Exceptions

| Rule | Explanation | Example |
|------|-------------|---------|
| **1. The aorist stem** | The same stem as the indicative, without the augment. | ἐπραξάμην → **πραξάμενος** |
| **2. ‑σάμενος / ‑όμενος** | First aorist with σα; second aorist with the thematic vowel. | λυσάμενος; πυθόμενος |
| **3. Declined like καλός** | First/second-declension adjective endings. | λυσαμένη, λυσαμένων |
| **4. Priority in time** | Usually an action prior to the main verb. | λυσάμενος … ἀπῆλθεν |
| **5. Present ≠ aorist** | λυόμενος (in progress) ≠ λυσάμενος (single act). | — |
| **6. The middle meaning** | "For oneself"; in deponents, active. | δεξάμενος "having received" |

---

## 4. Example Sentences

| Greek | Translation |
|-------|-------------|
| **λυσάμενος τὸν αἰχμάλωτον, οἴκαδε ἦγεν αὐτόν.** | Having ransomed the captive, he led him home. |
| **χρήματα πραξάμενος, ἀπῆλθεν.** | Having collected his money, he left. |
| **δεξάμενοι τὰ δῶρα, ἐχάρησαν.** | Having received the gifts, they rejoiced. |
| **ταῦτα γενόμενα ὁρῶντες, ἐθαύμαζον.** | Seeing these things happen, they marveled. |

---

## 5. Usage & Context

1. **Circumstantial/adverbial** — a compressed clause of time or cause: "having done this for himself…"
2. **Attributive with the article** — ὁ λυσάμενος "the one who ransomed."
3. **In narrative** — the subject's prior action before the main verb; the prose writers' chief connective device.
4. **The middle nuance** — the subject has a stake in the result: πραξάμενος "having collected (for his own benefit)" vs. πράξας "having done."

---

## 6. Historical Note

The suffix **‑μενος** continues the Indo-European middle participle suffix (*‑m(e)no‑); combined with the sigmatic aorist marker it produced ‑σάμενος. The formation remained stable throughout the history of Greek, and even Modern Greek preserves its traces in passive participles.

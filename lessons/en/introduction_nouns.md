...
| Case | The noun's job | Where it turns up | Example | Transliteration |
|---|---|---|---|---|
| Nominative | subject of the verb | the one performing the action; also the complement of "to be" | ὁ λόγος | ho lógos |
| Genitive | possessor, source, whole | the noun another noun belongs to or comes from | τοῦ λόγου | toû lógou |
| Dative | indirect object, instrument, place | the one who benefits; the thing used; where or when | τῷ λόγῳ | tôi lógōi |
| Accusative | direct object | what the action reaches; also extent of space or time | τὸν λόγον | tòn lógon |
| Vocative | address | the person or thing being spoken to | ὦ φίλε | ô phíle |

Textbooks often translate the cases with a fixed formula — genitive as "of", dative as "to" or "for". That is a useful first approximation and no more: the case marks a *relationship*, and each language renders that relationship with whatever means it has, whether a preposition, a suffix, or word order. Learn what each case **does** in the Greek sentence, and let the translation follow from that rather than the other way round.
...
|---|---|---|---|---|
| Nominative | ὁ λόγος | ho lógos | οἱ λόγοι | hoi lógoi |
| Genitive | τοῦ λόγου | toû lógou | τῶν λόγων | tôn lógōn |
| Dative | τῷ λόγῳ | tôi lógōi | τοῖς λόγοις | toîs lógois |

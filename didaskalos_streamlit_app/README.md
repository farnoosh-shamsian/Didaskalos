...
4. Regenerate the content manifest, which lists every treebank and lesson file:

   ```
   py -3 build_content_manifest.py
   ```

5. Commit and push to `main` (manifest included). Note: `.gcloudignore` excludes `treebanks/`
   from the Cloud Build upload, so the deployed app reads corpora from GitHub raw — a new
   collection goes live only once it is pushed to `main`. The local folder is used only when
   running the app locally.

## How discovery finds the files

Listing what is in `treebanks/` and `lessons/` takes two sources, merged:

- `didaskalos_streamlit_app/content_manifest.json` — generated by `build_content_manifest.py`
  and **read from disk**, so it ships inside the container image and needs no network at all.
  Regenerate it whenever files are added or removed; it lives in the app folder because that is
...
showing its corpus name under "Unknown author". Building the picker now costs no requests at all,
and fetches retry briefly on 429/503.

When a discovery source does fail, the app says so in a caption above the picker naming the source
and the error, rather than silently showing a shorter list.

What still depends on GitHub being reachable: the treebank and lesson files themselves, which are
downloaded from raw when a textbook is built. A GitHub outage or a throttled egress address will
fail a build even though the picker looks healthy.
